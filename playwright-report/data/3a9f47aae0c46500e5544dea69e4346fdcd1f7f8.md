...
- Name: known-defects.spec.ts >> Known defects — problem_user >> BUG-006 the cart badge should clear once every item is removed
- Location: tests/known-defects.spec.ts:63:7

# Error details

```
Error: Expected the cart badge to show 0 item(s)

Expected the cart badge to show 0 item(s)
      ...
      - generic [ref=e72]:
        ...
        - generic [ref=e76]:
          ...
          - generic [ref=e81]:
            ...
            - button "Remove" [active] [ref=e83] [cursor=pointer]
      - generic [ref=e84]:
...
```ts
  ...
  5  |   readonly cartBadge: Locator;
  6  |   readonly burgerMenuButton: Locator;
  7  |   readonly logoutLink: Locator;
  8  | 
  9  |   constructor(page: Page) {
  10 |     this.cartLink = page.locator('[data-test="shopping-cart-link"]');
  11 |     this.cartBadge = page.locator('[data-test="shopping-cart-badge"]');
  12 |     this.burgerMenuButton = page.getByRole('button', { name: 'Open Menu' });
  13 |     this.logoutLink = page.locator('[data-test="logout-sidebar-link"]');
  14 |   }
  15 | 
  16 |   async openCart(): Promise<void> {
  17 |     await this.cartLink.click();
  18 |   }
  19 | 
  20 |   async getCartItemCount(): Promise<number> {
  21 |     if ((await this.cartBadge.count()) === 0) {
  22 |       return 0;
  23 |     }
  24 | 
  25 |     return Number(await this.cartBadge.innerText());
  26 |   }
  27 | 
  28 |   async expectCartItemCount(expected: number): Promise<void> {
  29 |     await expect
  30 |       .poll(() => this.getCartItemCount(), {
  31 |         message: `Expected the cart badge to show ${expected} item(s)`,
  32 |       })
> 33 |       .toBe(expected);
     |        ^ Error: Expected the cart badge to show 0 item(s)
  34 |   }
